    ...
    axios.get(apiUrl).then((res) => {
      axios.get(urlImg).then((result) => {
        ...
        setMovie({
          ...
          logo:
            result.data.logos.length > 0
              ? result.data.logos[0].file_path
              : null,
            ...
            <div className="absolute w-full top-[15%] md:top-[25%] p-2 md:p-8 text-white">
              {logo.length > 0 ? (
                <img
                  ...
                  alt={movie?.title}
                  className="w-[50%] md:w-[50%] mb-2"
                />
              ) : (
                <h1 className="text-sm md:text-3xl mb-3 whitespace-normal">
                  {movie?.title}
                </h1>
              )}
              <p className="w-[50%] text-xs md:text-sm text-gray-300">
                {movie?.tagline}
              </p>
              <div className="mt-5 flex">
                <Link to={"/watch/" + jenis + "/" + id}>
                  <button className="bg-gray-100 text-black rounded-sm p-1 md:p-2 text-xs mr-3 px-2 md:px-5 flex">
                    <BiPlay size={25} className="mr-3"></BiPlay>{" "}
                    <span className="mt-1 text-xs text-bold">Play</span>
                  </button>
                </Link>
                <button
                  className="text-white border rounded-sm p-1 md:p-2 text-xs md:px-5 flex"
                  onClick={() => setShowModal(true)}
                >
                  <BiInfoCircle size={25}></BiInfoCircle>{" "}
                  <span className="mt-1 text-bold ml-3 text-xs">
                    More Information
                  </span>
                </button>
              </div>
